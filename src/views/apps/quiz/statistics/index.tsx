...
const QuizStatistics: React.FC = () => {
  ...
  const fetchPerformanceData = useCallback(async () => {
    ...
    try {
      ...
      if (response.operation_status === "success") {
        setData(response as unknown as UserPerformanceDashboard);
      } else {
        const errorMsg =
  ...
  // Load data only once on mount, or when retryCount changes
  useEffect(() => {
    let isMounted = true;
    ...
    // Only depend on retryCount to trigger refetch
  ...
  // Memoize stable formatter and common options (before other useMemos that use them)
  const percentageFormatter = useMemo(() => getPercentageFormatter(), []);
  const commonChartOptions = useMemo(() => getCommonChartOptions(), []);
  ...
  }, [processedTimeData.dates, commonChartOptions, percentageFormatter]);
  ...
  // Memoize overall score calculation - MUST be before conditional returns
  const overall = useMemo(
    () => Math.min(Math.max(data?.overall_average_score || 0, 0), 100),
    [data?.overall_average_score]
  );

  // Memoize navigation callbacks - MUST be before conditional returns
  const handleNavigateToAdd = useCallback(() => {
    router.push(`/${locale}/apps/quiz/add`);
  }, [router, locale]);

  const handleNavigateToHistory = useCallback(() => {
    router.push(`/${locale}/apps/quiz/history`);
  }, [router, locale]);

  // Memoize radial chart options to prevent recreation - MUST be before conditional returns
